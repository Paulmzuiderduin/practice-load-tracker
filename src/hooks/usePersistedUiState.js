...
export const usePersistedUiState = ({
  ...
  useEffect(() => {
    ...
    try {
      const raw = localStorage.getItem(`practice_load_module_visibility_${sessionUser.id}`);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      `practice_load_module_visibility_${sessionUser.id}`,
  ...
  useEffect(() => {
    if (!sessionUser) return;
    try {
      const raw = localStorage.getItem(`practice_load_preferences_${sessionUser.id}`);
      const parsed = raw ? JSON.parse(raw) : {};
      setPreferences((prev) => ({
        ...prev,
        ...parsed,
        ...
        showInAppHints: parsed.showInAppHints != null ? parsed.showInAppHints : prev.showInAppHints,
        showStatTooltips:
  ...
  useEffect(() => {
    if (!sessionUser) return;
    localStorage.setItem(`practice_load_preferences_${sessionUser.id}`, JSON.stringify(preferences));
  ...
  useEffect(() => {
    ...
    try {
      const raw = localStorage.getItem(`practice_load_workspace_${sessionUser.id}`);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      `practice_load_workspace_${sessionUser.id}`,
      JSON.stringify({
  ...
  const navItems = useMemo(
    () =>
      moduleConfig.filter((item) => {
        if (item.advanced && preferences.uiMode !== 'advanced') return false;
        return item.alwaysVisible || moduleVisibility[item.key] !== false;
      }),
    [moduleConfig, moduleVisibility, preferences.uiMode]
  );
  ...
  useEffect(() => {
    ...
    const last = localStorage.getItem(`practice_load_last_tab_${sessionUser.id}`);
  ...
  useEffect(() => {
    if (!sessionUser || !preferences.rememberLastTab || !didApplyStartTab.current) return;
    localStorage.setItem(`practice_load_last_tab_${sessionUser.id}`, activeTab);
  ...
  const sidebarStorageKey = sessionUser
    ? `practice_load_sidebar_collapsed_${sessionUser.id}`
    : 'practice_load_sidebar_collapsed';
  ...
  const mobilePrimaryKeys = ['hub', 'sessions', 'load', 'injuries'];
