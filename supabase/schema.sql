...
create table if not exists teams (
  ...
  name text not null,
  quarter_length_minutes integer not null default 7,
  created_at timestamptz not null default now()
);

alter table teams add column if not exists quarter_length_minutes integer not null default 7;
alter table teams drop constraint if exists teams_quarter_length_minutes_check;
alter table teams add constraint teams_quarter_length_minutes_check
  check (quarter_length_minutes in (5, 6, 7, 8));
